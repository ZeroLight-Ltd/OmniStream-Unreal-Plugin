...
//#include "ZLCloudPluginWebRTCIncludes.h"
#include "Serialization/MemoryReader.h"

enum class ZLCLOUDPLUGIN_API EZLCloudPluginInputType : uint8
{
	RouteToWindow = 0,
	RouteToWidget = 1
};
...
/**
 * IZLCloudPluginInputHandler extends the IInputDevice interface. Setting the target viewport allows for
 ...
 */
class ZLCLOUDPLUGIN_API IZLCloudPluginInputHandler : public IInputDevice
{
public:
	/**
	 ...
	 * @param Handler The function called when this message type is received. This handler must take a single parameter (an TArray<FString>&) and have a return type of void
	 */
	virtual void RegisterMessageHandler(const FString& MessageType, const TFunction<void(TArray<FString>&)>& Handler) = 0;

	/**
	 * @brief Find the function to be called whenever the specified message type is received.
	 * 
	 * @param MessageType The human readable identifier for the message
	 * @return TFunction<void(TArray<FString>&)> The function called when this message type is received.
	 */
	virtual TFunction<void(TArray<FString>&)> FindMessageHandler(const FString& MessageType) = 0;

	/**
	 * @brief Set the input handlers type. This controls whether input is routed to widgets or windows
	 */
	virtual void SetInputType(EZLCloudPluginInputType InputType) = 0;
};